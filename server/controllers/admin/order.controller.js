...
module.exports.edit = async (req, res) => {
  ...
  try {
    ...
    if (!orderItem) {
      return res.status(400).json({ message: "OrderItem không tồn tại" });
    }
    ...
    // ===== GỬI MAIL KHI CONFIRMED =====
    if (oldStatus !== "confirmed" && status === "confirmed") {
      const transaction = await Transaction.findByPk(order.transactionId);
      if (!transaction) {
        return res.status(400).json("Không tìm thấy transaction");
      }
      ...
      const text = `Xin chào ${order.fullName},

Đơn hàng ${order.code} của bạn đã được xác nhận thành công.
Cảm ơn bạn đã tin tưởng và sử dụng dịch vụ của chúng tôi!`;

      await sendEmailCofirmOrderHelper.sendEmail(
        order.email,
        subject,
        text,
        orderForMail
...
module.exports.changeStatusOrder = async (req, res) => {
  ...
  if (!validStatus.includes(status)) {
    return res.status(400).json("Status không hợp lệ");
  }

  try {
    const order = await Order.findByPk(orderId);
    if (!order) return res.status(400).json("Order không tồn tại");

    if (order.status === "cancelled") {
      return res.status(400).json("Order đã bị hủy");
    }

    const transaction = await Transaction.findByPk(order.transactionId);
    if (!transaction) {
      return res.status(400).json("Transaction không tồn tại");
    }
    ...
    if (status === "confirmed") {
      ...
      sendEmailCofirmOrderHelper.sendEmail(
        order.email,
        "Xác nhận đặt tour",
        `Xin chào ${order.fullName}, đơn hàng ${order.code} đã được xác nhận.`,
        { ...order.toJSON(), amount: transaction.amount }
      );
    }
    res.json({ message: "Cập nhật trạng thái order thành công!" });
  } catch (err) {
    console.error(err);
    res.status(500).json("Lỗi khi thay đổi trạng thái order");
  }
};
